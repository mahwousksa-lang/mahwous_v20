...
# ─── عتبات المطابقة ───
MATCH_THRESHOLDS = {
    "exact": 97,        # تطابق تلقائي
    "high": 85,         # تطابق عالي
    "medium": 70,       # تطابق متوسط — مراجعة
    "low": 55,          # تطابق ضعيف — مشكوك
    "none": 0           # لا تطابق
}

# ─── عتبات حسب الفئة ───
CATEGORY_THRESHOLDS = {
    "perfume_famous": {"exact": 92, "medium": 75},
    "perfume_niche": {"exact": 85, "medium": 65},
    "oud_bakhoor": {"exact": 80, "medium": 60},
    "default": {"exact": 90, "medium": 70},
}

# ─── أعمدة الملفات ───
PRODUCT_COLUMNS = ["المنتج", "Product", "product", "product_name", "اسم المنتج", "Name", "name"]
PRICE_COLUMNS = ["السعر", "Price", "price", "سعر", "الثمن", "Unit Price", "unit_price"]
ID_COLUMNS = ["no", "NO", "No", "ID", "id", "رقم", "SKU", "sku", "الرقم", "رقم المنتج"]
